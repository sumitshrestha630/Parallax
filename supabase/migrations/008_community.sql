-- ============================================================
-- Rooted – Community feature: professionals + messages tables
-- + extend profiles with student context columns
...
-- Own-row policies (profiles was created in 001 with RLS enabled)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'own profile insert'
  ) THEN
    EXECUTE 'CREATE POLICY "own profile insert" ON public.profiles FOR INSERT WITH CHECK (auth.uid() = id)';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'own profile update'
  ) THEN
    EXECUTE 'CREATE POLICY "own profile update" ON public.profiles FOR UPDATE USING (auth.uid() = id)';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'professionals' AND policyname = 'professionals readable to authenticated'
  ) THEN
    EXECUTE 'CREATE POLICY "professionals readable to authenticated" ON public.professionals FOR SELECT TO authenticated USING (true)';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'professionals' AND policyname = 'professionals insert authenticated'
  ) THEN
    EXECUTE 'CREATE POLICY "professionals insert authenticated" ON public.professionals FOR INSERT TO authenticated WITH CHECK (true)';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'messages' AND policyname = 'own messages select') THEN
    EXECUTE 'CREATE POLICY "own messages select" ON public.messages FOR SELECT USING (auth.uid() = user_id)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'messages' AND policyname = 'own messages insert') THEN
    EXECUTE 'CREATE POLICY "own messages insert" ON public.messages FOR INSERT WITH CHECK (auth.uid() = user_id)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'messages' AND policyname = 'own messages update') THEN
    EXECUTE 'CREATE POLICY "own messages update" ON public.messages FOR UPDATE USING (auth.uid() = user_id)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'messages' AND policyname = 'own messages delete') THEN
    EXECUTE 'CREATE POLICY "own messages delete" ON public.messages FOR DELETE USING (auth.uid() = user_id)';
  END IF;
END $$;
...
-- ── Seed professionals directory ──────────────────────────────
INSERT INTO public.professionals (full_name, role, company, field, location, bio, linkedin_url, avatar_seed)
SELECT * FROM (VALUES
...
) AS v(full_name, role, company, field, location, bio, linkedin_url, avatar_seed)
