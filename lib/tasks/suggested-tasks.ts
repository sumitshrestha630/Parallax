...
const CURATED: Record<string, Omit<SuggestedTask, "task_key" | "skill_key" | "node_id">[]> = {
  web_found: [
    {
      title: "Diagram the request lifecycle",
      description: "Trace what happens from URL bar to HTML on screen — DNS, TLS, HTTP, browser parsing.",
      difficulty: "easy",
      xp_reward: 20,
      estimated_minutes: 35,
      learning_objective: "Explain the browser–server round trip without jargon dumps.",
      instructions: [
        "Draw boxes for browser, DNS, server, database (even if hypothetical).",
        "Annotate one GET request with status line + headers you’d expect.",
        "Write 4 bullets: what can fail at each hop.",
      ],
    },
    {
      title: "Compare HTTP methods you’ll actually use",
      description: "Hands-on: send GET vs POST (curl, Postman, or fetch) against a public API.",
      difficulty: "medium",
      xp_reward: 40,
      estimated_minutes: 45,
      learning_objective: "Know when GET/POST/PATCH/DELETE apply to real endpoints.",
      instructions: [
        "Pick a public JSON API (e.g. GitHub, JSONPlaceholder).",
        "Issue a GET and capture response shape.",
        "Issue a POST with a tiny JSON body; note status codes.",
        "Summarize idempotency in your own words.",
      ],
    },
  ],
  html_css: [
